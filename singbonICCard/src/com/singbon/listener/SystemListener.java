...
public class SystemListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent arg0) {
		...
		UpgradeService upgradeService = (UpgradeService) app.getBean("upgradeService");
		...
		// 注册监控平台Co和读卡器升级通道、添加公司id到批次和黑名单映射监控平台用
		List<Company> companyList = companyService.selectAllList();
		for (Company c : companyList) {
			TerminalManager.registChannel("CR" + c.getId());
		...
		// 添加升级文件映射
		List<Upgrade> upgradeList = upgradeService.selectList();
		for (Upgrade u : upgradeList) {
			StringUtil.loadUpgradeFile(arg0.getServletContext().getRealPath("\\") + "/upgrade/", u);
		}

		JdbcUtil.baseDAO.initWeixinShortSystemStart();
