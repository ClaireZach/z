...
public class PosExecCommandDispatch {
	...
	public static void execCommand(InetSocketAddress inetSocketAddress, byte[] b) {
		...
		// 命令码
		// int commandCode = StringUtil.hexToInt(34, 37, b);
		...
		if (b[30] == PosFrame.Status && b[31] == PosSubFrameStatus.SysStatus) {
			...
			if (TerminalManager.CompanyIdToMonitorRunningList.containsKey(device.getCompanyId()) && TerminalManager.CompanyIdToMonitorRunningList.get(device.getCompanyId())) {
				...
				if (device.getDeviceType() != 1) {
					...
					// Integer mode =
					// TerminalManager.CompanyIdToMonitorModeList.get(device.getCompanyId());
			...
			// 记录帧 1普通消费、2补助消费、9领取补助记录、10在线充值记录、39补助清零
		} else if (b[30] == 1 && (b[31] == 1 || b[31] == 2 || b[31] == 3 || b[31] == 9 || b[31] == 0x11 || b[31] == 0x27)) {
			...
			// 在线充值请求
		} else if (b[30] == (byte) 0xDE && b[31] == 1) {
			PosExecChargeRequest request = new PosExecChargeRequest(device, b);
			request.run();
			// 补助请求
			...
			PosExecReplyCommand.execReplyCommand(device, commandCode, b, map, inetSocketAddress);
			return;
		}
	}
