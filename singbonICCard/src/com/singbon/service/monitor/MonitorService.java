...
public class MonitorService {
	 ...
	 * @throws Exception
	 */
	private static void time(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void grantSubsidy(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void disableSubsidy(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void getStatus(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void consumeParam(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void meal(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void discount(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void sysPwd(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void companyName(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void orderTime(Device device, InetSocketAddress inetSocketAddress, byte subFrame, SendCommand command, Integer beginIndex, Integer endIndex) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void updateCookbook(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void appendCookbook(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void getLastNumCookbook(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void modifyCookbook(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
		Cookbook cookbook = sendCommand.getCookbook();
	 ...
	 * @throws Exception
	 */
	private static void batchUpdate(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void batchAppend(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void blackUpdate(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void blackAppend(Device device, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void sysInit(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void upgradeStart(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
		String sendBufStr = StringUtil.hexLeftPad(PosFrame.Sys04, 2) + StringUtil.hexLeftPad(PosSubFrameSys04.UpgradeStart, 2) + "0000" + StringUtil.hexLeftPad(commandCode, 4) + "0000";
		String bufLen = StringUtil.hexLeftPad(2 + sendBufStr.length() / 2, 4);
		sendBufStr = device.getSn() + StringUtil.hexLeftPad(device.getDeviceNum(), 8) + CommandDevice.NoSubDeviceNum + DeviceType.Main + DeviceType.getDeviceTypeFrame(device) + bufLen + sendBufStr;
		byte[] sendBuf = StringUtil.strTobytes(sendBufStr);
	 ...
	 * @throws Exception
	 */
	private static void upgradeAppend(Device device, InetSocketAddress inetSocketAddress, SendCommand command) throws Exception {
		int upgradeIndex = command.getUpgradeIndex();
		int index = upgradeIndex / 300 + (upgradeIndex % 300 > 0 ? 1 : 0);
		if (index >= command.getUpgradeCount()) {
			index = command.getUpgradeCount() - 1;
		}
		List<String> list = TerminalManager.FilenameToUpgradeStrsList.get(device.getUpgradeType());

		String upgradeStr = list.get(index);
		String sendBufStr = StringUtil.hexLeftPad(PosFrame.Sys04, 2) + StringUtil.hexLeftPad(PosSubFrameSys04.UpgradeAppend, 2) + "0000" + StringUtil.hexLeftPad(command.getCommandCode(), 4)
				+ StringUtil.hexLeftPad(upgradeIndex, 8) + upgradeStr + "0000";
		String bufLen = StringUtil.hexLeftPad(2 + sendBufStr.length() / 2, 4);
		sendBufStr = device.getSn() + StringUtil.hexLeftPad(device.getDeviceNum(), 8) + CommandDevice.NoSubDeviceNum + DeviceType.Main + DeviceType.getDeviceTypeFrame(device) + bufLen + sendBufStr;
		byte[] sendBuf = StringUtil.strTobytes(sendBufStr);
		TerminalManager.sendToPos(inetSocketAddress, sendBuf);
	 ...
	 * @throws Exception
	 */
	public static void upgradeSync(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
		String sendBufStr = StringUtil.hexLeftPad(PosFrame.Sys04, 2) + StringUtil.hexLeftPad(PosSubFrameSys04.UpgradeSync, 2) + "0000" + StringUtil.hexLeftPad(commandCode, 4) + "0000";
		String bufLen = StringUtil.hexLeftPad(2 + sendBufStr.length() / 2, 4);
		sendBufStr = device.getSn() + StringUtil.hexLeftPad(device.getDeviceNum(), 8) + CommandDevice.NoSubDeviceNum + DeviceType.Main + DeviceType.getDeviceTypeFrame(device) + bufLen + sendBufStr;
		byte[] sendBuf = StringUtil.strTobytes(sendBufStr);
	 ...
	 * @throws Exception
	 */
	private static void upgradeEnd(Device device, InetSocketAddress inetSocketAddress, Integer commandCode) throws Exception {
		String sendBufStr = StringUtil.hexLeftPad(PosFrame.Sys04, 2) + StringUtil.hexLeftPad(PosSubFrameSys04.UpgradeEnd, 2) + "0000" + StringUtil.hexLeftPad(commandCode, 4) + "0000";
		String bufLen = StringUtil.hexLeftPad(2 + sendBufStr.length() / 2, 4);
		sendBufStr = device.getSn() + StringUtil.hexLeftPad(device.getDeviceNum(), 8) + CommandDevice.NoSubDeviceNum + DeviceType.Main + DeviceType.getDeviceTypeFrame(device) + bufLen + sendBufStr;
		byte[] sendBuf = StringUtil.strTobytes(sendBufStr);
	...
	public static void excuteCommand(Device d, InetSocketAddress inetSocketAddress, SendCommand sendCommand) throws Exception {
		...
		case PosFrame.Sys04:
			if (sendCommand.getSubFrame() == PosSubFrameSys04.ConsumeParam) {
				...
				map.put("des", "发送折扣费率命令");
			} else if (sendCommand.getSubFrame() == PosSubFrameSys04.SetCompanyName) {
				companyName(d, inetSocketAddress, sendCommand);
				map.put("des", "发送下载单位名称命令");
			} else if (sendCommand.getSubFrame() == PosSubFrameSys04.SysPwd) {
				sysPwd(d, inetSocketAddress, sendCommand);
				map.put("des", "发送下载系统密码命令");
			} else if (sendCommand.getSubFrame() == PosSubFrameSys04.UpgradeStart) {
				upgradeStart(d, inetSocketAddress, sendCommand.getCommandCode());
				map.put("des", "发送进入升级模式命令");
			} else if (sendCommand.getSubFrame() == PosSubFrameSys04.UpgradeAppend) {
				upgradeAppend(d, inetSocketAddress, sendCommand);
			} else if (sendCommand.getSubFrame() == PosSubFrameSys04.UpgradeEnd) {
				upgradeEnd(d, inetSocketAddress, sendCommand.getCommandCode());
				map.put("des", "发送升级完毕命令");
			}
		...
		case PosFrame.Cookbook:
			if (sendCommand.getSubFrame() == PosSubFrameCookbook.Update) {
				...
				Cookbook cookbook = sendCommand.getCookbook();
				String log = String.format("发送菜肴清单追加命令：第%s/%s个，编号：%s，单价：%s，菜名：%s", sendCommand.getCookbookIndex(), sendCommand.getCookbookTotal(), cookbook.getCookbookCode(),
				...
				map.put("des", "发送追加菜肴清单初始化命令");
			} else if (sendCommand.getSubFrame() == PosSubFrameCookbook.Modify) {
				modifyCookbook(d, inetSocketAddress, sendCommand);
				Cookbook cookbook = sendCommand.getCookbook();
				String log = String.format("发送修改菜肴命令：编号：%s，单价：%s，菜名：%s", cookbook.getCookbookCode(), cookbook.getPrice(), cookbook.getCookbookName());
				map.put("des", log);
		...
		if ("sysTime".equals(cmd)) {
			...
			for (SendCommand sc : sendCommandList) {
				if (sc.getFrame() == PosFrame.Cookbook
					...
					map.put("time", StringUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"));
					map.put("from", device.getDeviceName());
					map.put("des", "已经添加过菜单下载命令，不能重复添加");
					TerminalManager.sendToMonitor(map, company.getId());
					return;
				}
			}
			...
			if (cookbook == null) {
				...
				map.put("time", StringUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"));
				map.put("from", device.getDeviceName());
				String des = String.format("不存在编号为%s的菜单", cookbookCode);
				map.put("des", des);
				TerminalManager.sendToMonitor(map, company.getId());
				return;
			}
			...
			if (!TerminalManager.FilenameToUpgradeStrsList.containsKey(device.getUpgradeType())) {
				...
				map.put("time", StringUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"));
				map.put("from", device.getDeviceName());
				String des = String.format("不存在升级文件%s", device.getUpgradeType());
				map.put("des", des);
				TerminalManager.sendToMonitor(map, company.getId());
				return;
			}
			...
			for (SendCommand sc : sendCommandList) {
				if (sc.getFrame() == PosFrame.Sys04
						&& (sc.getSubFrame() == PosSubFrameSys04.UpgradeStart || sc.getSubFrame() == PosSubFrameSys04.UpgradeAppend || sc.getSubFrame() == PosSubFrameSys04.UpgradeEnd)) {
					Map map = new HashMap();
					map.put("type", "log");
					map.put("time", StringUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"));
					map.put("from", device.getDeviceName());
					map.put("des", "已经添加过升级命令，不能重复添加");
					TerminalManager.sendToMonitor(map, company.getId());
					return;
				}
			}
			...
			SendCommand sendCommand = new SendCommand();
			sendCommand.setFrame(PosFrame.Sys04);
			sendCommand.setSubFrame(PosSubFrameSys04.UpgradeStart);
			sendCommand.setCommandCode(0);
			sendCommand.setUpgradeIndex(0);
			sendCommand.setUpgradeCount(upgradeList.size());
			int size = TerminalManager.FilenameToUpgradeSizeList.get(device.getUpgradeType());
			sendCommand.setUpgradeTotalSize(size);

			sendCommandList.add(sendCommand);

			// // 非中转设备不添加升级和返回应用程序命令
			// if (device.getTransferId() == null || device.getTransferId() ==
			// 0)
			// return;
			//
			// List<String> upgradeList =
			// TerminalManager.FilenameToUpgradeStrsList.get(device.getUpgradeType());
			// for (int i = 0; i < upgradeList.size(); i++) {
			// SendCommand sendCommand2 = new SendCommand();
			// sendCommand2.setFrame(PosFrame.Sys04);
			// sendCommand2.setSubFrame(PosSubFrameSys04.UpgradeAppend);
			// sendCommand2.setCommandCode(i);
			// sendCommandList.add(sendCommand2);
			// }
			//
			// SendCommand sendCommand3 = new SendCommand();
			// sendCommand3.setFrame(PosFrame.Sys04);
			// sendCommand3.setSubFrame(PosSubFrameSys04.UpgradeEnd);
			// sendCommand3.setCommandCode(1000);
			// sendCommandList.add(sendCommand3);
		}
	}
}
