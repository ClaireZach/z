...
public class MainCardService extends BaseService {
	...
	public void makeCardByUserInfo(SysUser sysUser, Device device, SocketChannel socketChannel, User user, User cardUser, CardAllInfo cardAllInfo, int commandCode, Integer section) throws Exception {
		...
		if (commandCode == CardReaderCommandCode.RemakeCard) {
			...
			cardRecord.setOpFare(new BigDecimal(0));
			cardRecord.setOddFare(user.getOddFare());
			cardRecord.setSubsidyOddFare(user.getSubsidyOddFare());
			cardRecord.setCardOddFare(new BigDecimal(0));
			cardRecord.setCardSubsidyOddFare(new BigDecimal(0));
		...
		if (commandCode == CardReaderCommandCode.SingleCard || commandCode == CardReaderCommandCode.InfoCard) {
			...
			cardRecord.setOddFare(new BigDecimal(0));
			cardRecord.setSubsidyOddFare(new BigDecimal(0));
			cardRecord.setCardOddFare(new BigDecimal(0));
			cardRecord.setCardSubsidyOddFare(new BigDecimal(0));
			cardRecord.setOpCount(user.getOpCount());
...
//			cardRecord.setOpFare(opFare);
			BigDecimal opFare = (user.getPreOpFare().subtract(user.getCardDeposit())).multiply(new BigDecimal(100));
			cardRecord.setOpFare(opFare);
			this.cardRecordService.insert(cardRecord);

			// 赠送金额
			if (user.getGiveFare().intValue() > 0) {
				cardRecord.setRecordType(CardRecord.MakeCardGiveFare);
				//cardRecord.setOpFare(user.getGiveFare() * 100);
				cardRecord.setOpFare(user.getGiveFare().multiply(new BigDecimal(100)));
				this.cardRecordService.insert(cardRecord);
			}

			// 收取卡押金
			if (user.getCardDeposit().intValue() > 0) {
				cardRecord.setRecordType(CardRecord.GetCardDeposit);
				//cardRecord.setOpFare(user.getCardDeposit() * 100);
				cardRecord.setOpFare(user.getCardDeposit().multiply(new BigDecimal(100)));
				this.cardRecordService.insert(cardRecord);
			}
		}
		// 添加卡操作记录：按库修正
		...
		String baseBlock1 = baseInfoSection + "0100" + StringUtil.strToGB2312(user.getUsername()).substring(0, 32);
...
//		cardRecord.setCardSubsidyOddFare((long) 0);
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void unloss(SysUser sysUser, User user, BigDecimal dbOddFare, BigDecimal dbSubsidyOddFare, Integer dbOpCount, Integer dbSubsidyOpCount, SocketChannel socketChannel, Device device, String cardInfoStr)
			throws Exception {
...
//		cardRecord.setSubsidyOddFare(dbSubsidyOddFare);
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void offUserInfoWithInfo(SysUser sysUser, User user, BigDecimal dbOddFare, BigDecimal dbSubsidyOddFare, Integer dbOpCount, Integer dbSubsidyOpCount) throws Exception {
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void updateByCard(SysUser sysUser, User user, BigDecimal dbOddFare, BigDecimal dbSubsidyOddFare, Integer dbOpCount, Integer dbSubsidyOpCount) throws Exception {
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void doCharge(SysUser sysUser, User user, Integer chargeType, BigDecimal opFare, BigDecimal cardOddFare, BigDecimal cardSubsidyOddFare, Integer cardOpCount, Integer cardSubsidyOpCount,
		...
		BigDecimal cardTotalFare =  new BigDecimal(Integer.parseInt(totalFareString, 16));
		// 卡总操作余额
		BigDecimal cardAllOddFare = new BigDecimal(0);

		// 存款
		if (chargeType == 0) {
//			cardAllOddFare = (int) (cardOddFare + (100 * (opFare + user.getGiveFare())));
//			cardTotalFare += (opFare + user.getGiveFare()) * 100;
//			this.userDAO.changeFare(user.getUserId(), (int) (opFare + user.getGiveFare()) * 100, false);
			
			cardAllOddFare = opFare.add(user.getGiveFare()).multiply(new BigDecimal(100)).add(cardOddFare);
			cardTotalFare  = cardTotalFare.add(opFare.add(user.getGiveFare()).multiply(new BigDecimal(100)));
			this.userDAO.changeFare(user.getUserId(), opFare.add(user.getGiveFare()).multiply(new BigDecimal(100)), false);
			

			// 添加卡操作记录
			CardRecord cardRecord = new CardRecord();
			cardRecord.setCompanyId(sysUser.getCompanyId());
			cardRecord.setOperId(sysUser.getOperId());
			cardRecord.setUserId(user.getUserId());
			cardRecord.setCardNO(user.getCardNO());
			cardRecord.setCardSN(user.getCardSN());
			...
			cardRecord.setOpFare(opFare.multiply(new BigDecimal(100)));
					...
					try {
						...
						if (!StringUtils.isEmpty(openIdsAndMobiles.getOpenIds())) {
							...
							for (String openId : openIds) {
								...
								sb.append("\"data\":{");
								sb.append("\"keyword1\":{\"value\":\"" + openIdsAndMobiles.getUsername() + "\",\"color\":\"#173177\"},");
								sb.append("\"keyword2\":{\"value\":\"" + opFare + "\",\"color\":\"#173177\"},");
								sb.append("\"keyword3\":{\"value\":\"" + chargeTime + "\",\"color\":\"#173177\"},");
								sb.append("\"keyword4\":{\"value\":\"PC充值成功\",\"color\":\"#173177\"}");
								sb.append("}");
								sb.append("}");
					...
					try {
						...
						if (!StringUtils.isEmpty(openIdsAndMobiles.getMobiles())) {
							...
							for (String mobile : mobiles) {
								...
								String msg = String.format("尊敬的%s家长，您的学生校园卡充值%s元，充值时间%s", openIdsAndMobiles.getUsername(), opFare, chargeTime);
...
//			boolean off = "on".equals(backCardDeposit) ? true : false;
//			this.userDAO.changeFare(user.getUserId(), -(int) (opFare * 100), off);
			
			cardAllOddFare = (cardOddFare.subtract(opFare.multiply(new BigDecimal(100))));
			cardTotalFare = cardTotalFare.subtract(opFare.multiply(new BigDecimal(100)));
			boolean off = "on".equals(backCardDeposit) ? true : false;
			this.userDAO.changeFare(user.getUserId(), opFare.multiply(new BigDecimal(100)).multiply(new BigDecimal(-1)), off);
			...
			if ("on".equals(backCardDeposit) && user.getCardDeposit().intValue() > 0) {
	...
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void doImport(MultipartFile file, Integer companyId, Integer deptId) throws Exception {
		try {
			...
			for (int i = 1; i <= rowNum; i++) {
				...
				if (userNO.trim().length() == 0)
					break;
				int count = this.selectCountByUserNO(companyId, userNO);
				if (count > 0) {
					throw new RuntimeException("已存在编号" + userNO + "的信息");
				}
				String username = StringUtil.objToString(row.getCell(1));
				String sex = StringUtil.objToString(row.getCell(2));
				int gender = "女".equals(sex) ? 1 : 0;
				String cardID = StringUtil.objToString(row.getCell(3));
				int cardTypeId = StringUtil.objToInt(row.getCell(4));
				String mobile1 = StringUtil.objToString(row.getCell(5));
				String mobile2 = StringUtil.objToString(row.getCell(6));
				
				/************************************************************/
				// TODO add giveFare and preFare 2016.08.29 zch 
				String giveFare = StringUtil.objToString(row.getCell(7));
				String preFare = StringUtil.objToString(row.getCell(8));
				/************************************************************/
				...
				user.setShortName(StringUtil.toJP(username));
				user.setSex(gender);
				user.setCardID(cardID);
				user.setDeptId(deptId);
				user.setStatus(0);
				user.setCardTypeId(cardTypeId);
				user.setConsumePwd("8888");
				...
				/*****************************************/
				// TODO add giveFare and preFare 2016.08.29 zch 
				user.setGiveFare(new BigDecimal(giveFare));
				user.setPreOpFare(new BigDecimal(preFare));
				/*****************************************/
