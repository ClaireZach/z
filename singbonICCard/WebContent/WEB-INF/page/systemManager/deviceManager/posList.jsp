	...
	<table class="table" width="99%" layoutH="148">
		...
		<tbody>
			<c:forEach items="${deviceList }" var="d" varStatus="status">
				<tr target="posList" id="${d.id }" deviceNum="${d.deviceNum}" deviceName="${d.deviceName}" deviceType="${d.deviceType}" transferId="${d.transferId}" paramGroupId="${d.paramGroupId}" sn="${d.sn}" enable="${d.enable}" entranceType="${d.entranceType}">
					...
					<td>${d.sn}</td>
					<td>
						<c:if test="${d.entranceType==1}">进门</c:if>
						<c:if test="${d.entranceType==2}">出门</c:if>
						<c:if test="${d.entranceType==3}">进出门</c:if>
					</td>
