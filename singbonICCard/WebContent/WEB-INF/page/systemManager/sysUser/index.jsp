...
<script type="text/javascript">
			...
			if(form.valid()){
				...
				validateCallback(form, function(e) {
					if (e == 1) {
						...
						$('#sysUserForm input[name=loginName]').removeClass('disabled').removeAttr('readonly');
						form.clearForm();
						refreshsysUserList();
					} else {
						alertMsg.error('修改失败！');
	...
	function sysUserClick(tr) {
		...
		if(tr.attr('loginName')=='admin'){
			$('#sysUserForm input[name=loginName]').addClass('disabled').attr('readonly',true);
		}else{
			$('#sysUserForm input[name=loginName]').removeClass('disabled').removeAttr('readonly');
		}
	};
