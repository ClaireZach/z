...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#userInfo .confirm').click(function() {
			//非注销
			if(!$("input[name=backCardDeposit]").is(':checked')){
				if(getAllOddFare()){
					validateCallback($(this).parents('form'), function(e) {
					}, null);
				}
			}else{
				alertMsg.confirm('确定注销卡片吗？', {
					okCall : function() {
						$.post('${base }/doChangeCard.do?editType=3&cardSN='+$('#userInfo input[name=cardSN]').val());
					}
				});
			}
		});
		...
		$('#userInfo input[name=backCardDeposit]').click(function(){
			if($(this).is(':checked')){
				$('#userInfo input[name=opFare]').attr('readonly',true);
			}else{
				$('#userInfo input[name=opFare]').removeAttr('readonly');
			}
		});
	});
	...
	<div class="formBar">
		<ul>
			<li id="chargeLi" style="display: none;"><div class="button">
					<div class="buttonContent confirm">
