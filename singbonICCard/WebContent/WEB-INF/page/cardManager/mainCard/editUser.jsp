...
<script type="text/javascript">
	...
	$(function() {
		...
		//alert($('#headHurtA').val()/100.00);
		
		$('#headHurtA').val($('#headHurtA').val()/100.00);
		$('#headHurtB').val($('#headHurtB').val()/100.00);
	...
	function batchCardClick(tr){
		...
		if(status==0){
			...
			$('#userinfo input[name=deptId]').val(tr.attr('deptId'));
			$('#userinfo input[name=cardSN]').val('');
	...
	function getAllFare(validMinus){
...
// 			giveFareInput.val(0);
		...
		allFare=(parseFloat(preOpFare)+parseFloat(giveFare)-parseFloat(cardDeposit)).toFixed(2);
		allFareInput.val(allFare);
		if(allFare>167772.15){
			alertMsg.warn('实际金额不能大于167772.15！');
			return false;
		}
// 		if(validMinus && allFare<=0){
// 			alertMsg.warn('实际金额应大于0！');
// 			return false;
// 		}
		return true;
	}
		...
		<tbody>
			<c:forEach var="user" items="${list}" varStatus="status">
				<tr target="batchCard" <c:if test="${status.index==0}">class="selected"</c:if> 
					userId="${user.userId}" status="0" deptId="${user.deptId}" username="${user.username}" shortName="${user.shortName}" userNO="${user.userNO}" cardTypeId="${user.cardTypeId}"
	...
	<div class="pageFormContent" layoutH="60">
		<fieldset>
			...
			<dl>
				...
				<dd>
					...
					<input name="username" type="text" class="required"
						...
						onkeydown="shortName.value=($('input[name=username]').toPinyin())" placeholder="请输入姓名"/>
				</dd>
			</dl>
			<dl>
				<dt>姓名简拼：</dt>
			...
			<dl>
				<dt>编号：</dt>
				<dd>
					<input class="required" name="userNO" type="text" value="${user.userNO }" <c:if test="${status!=0}">readonly="readonly"</c:if> placeholder="请输入编号"/>
				</dd>
			</dl>
			<dl>
				<dt>性别：</dt>
			...
			<dl>
				<dt>绑定手机1：</dt>
				<dd>
					<input name="mobile1" type="text" value="${user.mobile1}" placeholder="请输入绑定手机号1"/>
				</dd>
			</dl>
			<dl>
				<dt>绑定手机2：</dt>
				<dd>
					<input name="mobile2" type="text" value="${user.mobile2}" placeholder="请输入绑定手机号2"/>
				</dd>
			</dl>
		</fieldset>
		<fieldset>
			...
			<c:if test="${editType==5 || editType==6}">
				...
				<dl>
					...
					<dd>
						...
						<%-- <input type="text" value="${user.oddFare/100.00 }" readonly="readonly"/> --%>
						<input type="text" id="headHurtA" value="${user.oddFare}" readonly="readonly"/>
					</dd>
				</dl>
				<dl>
					<dt>补助余额：</dt>
					<dd>
						...
						<%-- <input type="text" value="${user.subsidyOddFare/100.00 }" readonly="readonly"/> --%>
						<input type="text" id="headHurtB" value="${user.subsidyOddFare}" readonly="readonly"/>
					</dd>
				</dl>			
			</c:if>
			<c:if test="${editType!=5 && editType!=6 && status==0}">
				<dl>
					<dt>实际金额：</dt>
					<dd>
				...
				<dl>
					<dt>赠送金额：</dt>
					<dd>
						<input class="required number" name="giveFare" type="text" value="${giveFare}" onkeyup="getAllFare(false);" placeholder="请输入预发金额"/>
					</dd>
				</dl>
				<dl>
					<dt>预发金额：</dt>
					<dd>
						<input class="required number" name="preOpFare" type="text" value="${preOpFare}" onkeyup="getAllFare(false);" placeholder="请输入赠送金额"/>
					</dd>
				</dl>
			</c:if>
		</fieldset>
