...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 10 : -5) - 5;
	var iContentH = $(window).height() - $("#header").height() - 3;

	$("#container").width(iContentW);
	$("#container .tabsPageContent").height(iContentH).find("[layoutH]").layoutH();
		...
		$(this).click(function(event){
			...
			if(url!=null){
				...
				if(url.indexOf('funcCard/index.do')!=-1){
					$.pdialog.open(url, 'dialog', '功能卡制作', options);
				}else if(url.indexOf('cardReader/param.do')!=-1){
					$.pdialog.open(url, 'dialog', '参数下载', options);					
				}else if(url.indexOf('cardReader/upgrade.do')!=-1){
					options['width']=500;
					options['height']=300;
					$.pdialog.open(url, 'dialog', '设备升级', options);					
				}
				return;
			}
